...
//fft set-up
16184 => fft.size => int FFT_SIZE;//frequency resolution
2048 => int WINDOW_SIZE;//time domain samples
...
//an array for polyphonic pitch detection
int B[];
0.008 => float Thresh;
...
while (true){
    ...
    float Z[FFT_SIZE/10];//if sample rate is 96000 then this analyses 0 - 9k
    ...
    if(rmsInfo > rmsThreshold){
      ...
      255 - percentOff/(0.029/255) => xmit.addFloat;   
       ...
       //Prints Out User Feedback Data
        ...
        <<<"">>>;
        //if you are close it congratulates you if you are within two cents of the correct tone
        if ((freq < Std.mtof(targetFreq) +2) && (freq > Std.mtof(targetFreq) - 2)){
...
fun void MaxPeaks(float A[], int B[], float Thresh){
    float LP[A.cap()];
    0.0 => float tempMaxValue;
    0 => int tempMaxIndex;
    for ( 0=> int i; i < A.cap() -1; i++){
...
fun float Bin2freq(int bin, float sr, int fftsize){
    float freq;
    ((bin * sr) / fftsize) * 2 => freq;//it was returing the wrong freq's for some reason
    return freq;   
}

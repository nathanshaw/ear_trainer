...
["G","G#","A","A#","B","C","C#","D","D#","E","F","F#"] @=> string pianoNoteNames[];
["Unison","Major Second", "Major Third","Perfect Fourth", "Perfect Fifth", "Major Sixth", "Major Seventh","Octave"] @=> string ascendingIntervalString[];
["Unison","Minor Second", "Minor Third","Perfect Fourth","Perfect Fifth", "Minor Sixth", "Minor Seventh","Octave"] @=> string descendingIntervalString[];
//["Perfect Fourth", "Perfect Fifth", "Octave"] @=> string intervalsPerfect;
...
fun int targetFrequency(int startTone){
    <<<"Starting Tone Freq: ", startTone>>>;
    (startTone - 55) % 12 => int noteIndex;
    <<<"Starting Tone Name: ",pianoNoteNames[noteIndex] >>>;
    //piano_samples[tonePlaceholder] => piano.read;
    //<<<"target frequency function running">>>;
    //0 => piano.pos;
    Math.random2(0,1) => adChance;
    if(startTone >= 60){
        Math.random2(0,7) => int toneIndex;
        startTone - descendingIntervals[toneIndex] => startTone;
        <<<"Please sing a descending",descendingIntervalString[toneIndex]," from the given tone">>>;
        startTone => toneIndex;
         (startTone - 55) % 12 => noteIndex;
        return startTone;
        <<<"Your Target Note is :",pianoNoteNames[noteIndex]>>>; 
        //<<<"over 63">>>;
    }
    if(startTone < 60){
        Math.random2(0,7) => toneIndex;
        startTone + ascendingIntervals[toneIndex] => startTone; 
        
        //Std.mtof(targetInterval) => targetFreq; 
        //<<<"Please sing an ascending",ascendingIntervalString[toneindex]," from the given tone.">>>;
         (startTone - 55) % 12 => noteIndex;
          <<<"Your Target Note is :",pianoNoteNames[noteIndex]>>>; 
        startTone => toneIndex;
        return startTone;
